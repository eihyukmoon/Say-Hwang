    ...
    def assemble(self, text, output_path="final_output_hybrid.mp3"):
        ...
        for word in words:
            ...
            if prefix_chunk:
                ...
                if clip:
                    if len(combined) > 0: combined = combined.append(clip, crossfade=10)
                    else: combined += clip

            # B. Middle (한 글자씩) [수정됨: 복잡한 속도 조절 제거, 단순 합치기]
            # 범위: Prefix 끝난 곳 ~ Suffix 시작 전까지
            ...
            for i in range(start_mid, end_mid):
                ...
                clip = self._get_clip_from_info(info) # [수정] 공통 메서드로 처리
            ...
            # C. Suffix 추가 [수정됨: 올바른 변수(suffix_chunk) 사용]
            if suffix_chunk:
